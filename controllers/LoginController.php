...
//	debug($a);
	...
	$ob->on(\app\models\Country::EVENT_BEFORE_INSERT, ['app\controllers\LoginController','methodName']);
	$ob->language_id=1;
	$ob->name='777';
	debug($ob->save());
...
public function methodName($e)
{
	...
	$e->sender->name=$e->sender->name."ananev";
}
public function actionWww()
{
	$obj=\app\models\Category::find();
	$dataProvider = new ActiveDataProvider([
            'query' => $obj,
        ]);
		$obj->andFilterWhere(['category_id'=>161]);
...
public function actionOk()
{
    ...
    $model = new \app\models\Guest();

    if ($model->load(Yii::$app->request->post())) {
        if ($model->validate()) {
         //   $model->fam=Yii::$app->request->post('Guest')['fam'];
		 debug(Yii::$app->request->queryParams);
